...
class EnvelopeSimulator:
    def __init__(self, root):
        self.root = root
        self.root.title("Linear Envelope Simulator")
        self.root.geometry("500x700")  # Increased height for new controls
        ...
        self.interval_sec = tk.DoubleVar(value=1.0)
        ...
        self.time_slider = ttk.Scale(root, from_=0, to=100, variable=self.time_val, command=self.update_envelope)
        ...
        self.interval_spin = ttk.Spinbox(clock_frame, from_=0.1, to=10, increment=0.1, width=8, textvariable=self.interval_sec)
    ...
    def audio_playback_loop(self):
        ...
        try:
            ...
            total_duration = self.decay_val.get() / 100.0 * 2.0  # Scale to 2 seconds max
    ...
    def advance_clock(self):
        ...
        step = 1
        new_time = current_time + step if current_time < 100 else 0
        ...
        if self.recording and self.clock_running:
            try:
                ...
                self.actual_shape.append((new_time, amp))
            except ValueError:
                pass
    ...
    def draw_envelope(self, A, D, t):
        ...
        if t >= 0:
            ...
            self.canvas.create_text(current_x, current_y - 15, text=f"t={t:.1f}\na={current_amp:.2f}",
